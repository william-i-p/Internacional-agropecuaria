<?php
if(!class_exists("HTSliderProBase")) {
	class HTSliderProBase {
    	public $key = "A4DBAB422489F7A1";
    	private $product_id = "9";
    	private $product_base = "ht-slider-addons-pro";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_ht-slider-addons-pro_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=ht-slider-addons-pro_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
